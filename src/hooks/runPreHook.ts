...
type PreHookContext = {
	cline: Task
	input: RunPreHookInput
	workspaceRoot: string
	activeIntent: Awaited<ReturnType<typeof getCurrentActiveIntent>>
}

const preToolHookManager = new HookManager<PreHookContext>()

preToolHookManager
	.register({
		id: "require-active-intent",
		criticality: "critical",
		run: async ({ input, activeIntent }) => {
	...
	.register({
		...
		criticality: "critical",
		run: async ({ cline, input, activeIntent }) => {
	...
	.register({
		id: "intent-trace-bridge",
		criticality: "best_effort",
		run: async ({ cline, workspaceRoot, activeIntent }) => {
			if (!activeIntent) {
				return
			}
	...
	.register({
		id: "intent-state-sync",
		criticality: "best_effort",
		run: async ({ cline, input, workspaceRoot, activeIntent }) => {
			if (!activeIntent) {
				return
			}
...
export async function runPreHook(cline: Task, input: RunPreHookInput): Promise<void> {
	...
	await preToolHookManager.execute({
		cline,
		input,
		workspaceRoot,
		activeIntent,
